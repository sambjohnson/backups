...
def get_color_palette(nclasses):
    """ Creates a color map with nclasses + 1 (black cortical background)
        and returns the map as an np array.
        Arguments: nclasses, the number of labeled anatomical cortical ROIs (not including background).
    """
    sns_cp = sns.color_palette("husl", nclasses)
    sns_np_plus_bg = np.concatenate([[[0.0, 0.0, 0.0]], sns_cp])
    return sns_np_plus_bg


def get_filtered_destrieux(parc):
    """ This helper fuction returns a subset of the Destrieux labels that are
        concentrated around the OTS. This is used when pre-training
        on, e.g. HBN participants' Freesurfer Destrieux parcellations."""
    filtered_parc = np.array([DESTRIEUX_LABELS.index(l) if l in DESTRIEUX_LABELS else -1 for l in parc])
    return filtered_parc
...
def make_subject_images(mesh, curv, parc, nclasses,
                        extra_channels_dict=None,
                        parc_save_dir=None,
                        curv_save_dir=None,
                        angles=None,
                        nangles=20,
    ...
    """ Main image-generation function for UNet input images.
        Arguments:
            ...
            nclasses: number of anatomical labels to visualize (not including
                cortical background).
        Returns:
    ...
    cmap = get_color_palette(nclasses)
    vmax =
